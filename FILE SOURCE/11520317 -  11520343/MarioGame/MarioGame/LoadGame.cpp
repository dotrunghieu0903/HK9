...
BaseObject* BObject::Deserialize(const char *noidungcon)
{
	...
	if (GListObjects[ID] == NULL)
	{
		...
		case EObjectName::FLOWER_BLOCK_OBJECT:
			((SecretBlock *)obj)->_isCollision = isCollision;
			if (hasSubObject == false)
			{
				((SecretBlock *)obj)->_object = NULL;
			}
			break;
...
MarioPlayer* BMario::Deserialize(const char *noidungcon)
{
	CMarkup xml(noidungcon);
	xml.FindElem();
...
MarioPlayer* BSaveData::GetMarioPlayer()
{
	return this->_Mario;
}
...
MarioPlayer* LoadGame::GetMarioPlayer()
{
	return this->_Mario;
}
